...
- Deleting an empty block that was between two lists would throw an Error when all three elements were nested inside a
  list. #TINY-11763
...
- Fixed JavaScript error when inserting a table using the context menu by adjusting the event order
  in `renderInsertTableMenuItem`. #TINY-6887
...
- New `sandbox_iframes_exclusions` option that holds a list of URL host names to be excluded from iframe sandboxing
  when `sandbox_iframes` is set to `true`. #TINY-10350
...
- Zero width no-break space (U+FEFF) characters are removed from content passed to `setContent`, `insertContent`,
  and `resetContent` APIs. #TINY-10305
